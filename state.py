...
class State:
    ...
    def __init__(self, state=None):  # init with 0 arg or 1 arg (state)O
        if not state:
    ...
    def get_valid_moves(self):
        if self.previous_move != None:
            ...
            print("index_local_board", index_local_board)
            print("previous_move", self.previous_move)
        # first move
        else:
            temp_blocks = np.zeros((3, 3))
            indices = np.where(temp_blocks == 0)
            ret = []
            # loop through all local boards
            # and get empty cells
            for i in range(9):
                ret += [
