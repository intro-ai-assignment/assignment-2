...
def calc_score_global(cur_state, player, value_board=None, turn_amount=0):
    ...
    boards = np.array(
        [np.array([block for block in cur_state.blocks] * 3) for _ in range(3)])
    for large_y in range(3):
        for large_x in range(3):
            ...
            score += calc_twos(local_board,
                               value_board['2 in a row'], player, opponent)
    ...
    score += calc_twos(global_board,
                       value_board['won 2 in a row'], player, opponent)
    ...
    for y in range(len(boards)):
        for x in range(len(boards[y])):
            score += calc_block(boards[y][x],
                                value_board['blocked 2'], player, opponent)
            score -= calc_block(boards[y][x],
                                value_board['blocked 2'], opponent, player)

    # Won boards blocking score
    score += calc_block(global_board,
                        value_board['won block 2'], player, opponent)
    score -= calc_block(global_board,
                        value_board['won block 2'], opponent, player)
...
def new_minimax_ab(cur_state, depth, player, opponent, alpha=-np.inf, beta=np.inf, maximizing=True, starting=False,
    ...
    if cur_state.game_over or depth <= 0:
        score = calc_score_global(
            cur_state, player, None, turn_amount=initial_depth - depth)
        return score
    if maximizing:
        max_eval = -np.inf
        best_move = None
...
def select_move(cur_state, remain_time):
    ...
    if valid_moves:
        best_move = new_minimax_ab(cur_state, 2, cur_state.player_to_move, -
                                   cur_state.player_to_move, -np.inf,  np.inf, True, True, 81)[0]
        return best_move
    return None
